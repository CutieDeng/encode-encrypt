use std::{mem::{MaybeUninit, transmute}, marker::PhantomData, fmt::Display};
...
#[derive(Clone)]
pub struct Sha1 (pub [u32; 5], PhantomData<()>);
...
impl Sha1 {
    pub const fn new() -> Self {
        Sha1(init_variables(), PhantomData)
    }
    ...
    pub fn hash_block_with_cache(&mut self, values: &[u8; 64], cache: &mut [u32; 80]) {
        // let new_array: &[u32; 16] = unsafe { transmute(values) }; 
        // assert_eq ! ( new_array.len(), 16 ); 
        // for index in 0..16 {
        //     cache[index] = new_array[index]; 
        // }
        BigEndian::read_u32_into(values, &mut cache[..16]);
        // LittleEndian::read_u32_into(values, &mut cache[..16]);
        for index in 16..80 { 
